# @hexadrop/eslint-config-typescript agent guide

The typescript slice of the shared ESLint configuration, publishable and usable standalone.

- **Package manager:** Bun (`bun`)
- **Build:** `bun run --filter @hexadrop/eslint-config-typescript build` (from the repo root)
- **Test:** `bun test packages/eslint-config-typescript` (or `bun run test` at root)
