# @hexadrop/eslint-config-react agent guide

The react slice of the shared ESLint configuration, publishable and usable standalone.

- **Package manager:** Bun (`bun`)
- **Build:** `bun run --filter @hexadrop/eslint-config-react build` (from the repo root)
- **Test:** `bun test packages/eslint-config-react` (or `bun run test` at root)
