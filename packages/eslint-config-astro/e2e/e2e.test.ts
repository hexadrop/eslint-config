...
async function createESLint(e2eCase: EndToEndCase): Promise<{ eslint: ESLint; inputPath: string }> {
	...
	const overrideConfig = await astro(
		e2eCase.options,
		...((e2eCase.extraConfigs ?? []) as Parameters<typeof astro>[1][])
	);
 ...
 * halves of its behavior, mirroring packages/eslint-config-react:
 *
 * - `result.output ?? source`: the file content after `eslint --fix`, pinned
 *   so rule or parser regressions surface as snapshot diffs.
 * - `result.messages`: the diagnostics that survive the pass, pinned so rule
 *   wiring changes surface as snapshot diffs.
 */
